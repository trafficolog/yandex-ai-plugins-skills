...
Practitioner-informed release: five service plugins are strengthened with practical decision workflows reviewed from regularly used `artwist-polyakov/polyakov-claude-skills` at exact source SHA `f6a75133b3ad07e43991433c1d0a77f69c749f7b`.

### Changed

- Direct `2.2.0`: establish business objective and comparable data before KPI conclusions; rare/`RARELY_SERVED` demand is not automatically removed; recommendations follow observation → hypothesis → check → action → effect criteria.
- Metrika `2.2.0`: separate business conversions from micro-events, retain zero-conversion rows in denominators, compare periods on a compatible attribution/data basis, and require compatible spend/revenue provenance for PnL/ROAS.
- Search `1.1.0`: add an optional bounded smart-snippet/info-context synchronous mode with JSON normalization; ordinary sync/deferred SERP remains the default for rankings/batch/snapshots/clustering.
- Webmaster `2.2.0`: re-read diagnostics/quota/task state as volatile evidence; recrawl requires a concrete fixed/changed URL and evidence instead of blanket recrawl.
- Wordstat `1.2.0`: establish region and business offer before expansion, add intent classification plus selective Search validation of high-value/ambiguous clusters, and add missed-demand analysis against Direct semantics.
- Added semantic eval scenarios that exercise practical decisions rather than only string presence in documentation.
- Repaired a stale date-sensitive Project Memory test fixture; production Project Memory behavior is unchanged.
- Historical release tests are decoupled from the current plugin matrix and now validate historical release surfaces as history instead of blocking future minor releases.

### Safety and evidence boundary

- The existing exact-preview contract is unchanged: `yandex-ai-approval/v2`, exact `preview_id`, later-turn approval, `--ack-bulk`, and `yandex-ai-execution/v1` remain mandatory where applicable.
- External practitioner methodology supplements but does not replace official Yandex API contracts and is not evidence of causal or ranking effects.
- P3 remains `INFRASTRUCTURE_READY`; this release adds no accepted live multi-model evidence for `COMPARATIVE_COMPLETE`.
...
- Added `yandex-seo-weekly-report` with normative `seo-weekly-organic-report/v1`: exact current/comparison periods, explicit Webmaster/Metrika coverage, source quality/limitations, provenance, findings, and delegated recommendations.
- Added `yandex-ai-artifact-manifest/v1` with managed relative paths, media roles, SHA-256, and immutable snapshot semantics. Exact deterministic replay is allowed only when bytes/hashes match; a collision never overwrites an existing artifact set.
- `report.html` is self-contained with restrictive CSP, escaped source/user text, and no CDN, remote fonts, analytics, or network fetch; `report.json` remains the machine source of truth.
- Deterministic Mermaid/DOT exports are emitted only for structural/semantic/cluster/link structures that actually exist.
- Added transport-free `demo` and `build` CLI modes plus a sanitized offline fixture. `yandex-seo` receives no local Yandex credentials or HTTP transport.
- Optional P1 context is limited to project identity plus active `USER_STATED` facts; decisions, hypotheses, and stale baselines do not become fresh observed evidence and do not suppress source limitations.
- Every delegated action remains `PREVIEW-ONLY`; P2 does not expand P0 exact-preview/later-turn approval/bulk gates.
- `CONTRACT_MATRIX.json`, eval routing, RU/EN Architecture / Getting Started / Security / README / Service Matrix, and regression contracts are synchronized with P2.
...
P1 Project Memory is a repository-only release that ships project-owned domain memory for durable context across separate runs without becoming AI-runtime memory or expanding write authority.

### Changed

- Added the zero-third-party-dependency `scripts/ya_project.py` CLI and canonical `.yandex-ai/` scaffold: `project.yaml`, `decisions.jsonl`, `baselines/`, and `hypotheses.md`.
- `yandex-ai-project/v1` stores project identity plus `USER_STATED` facts; explicit supersession preserves history and prevents two active values for one logical key.
- `record-execution` projects P0 `yandex-ai-execution/v1` into chained `yandex-ai-decision/v1`: raw `result` is not persisted and `receipt_sha256` is computed over the complete source receipt.
- `add-baseline` creates immutable `yandex-ai-baseline/v1` snapshots; exact `fresh_until` remains `FRESH`, while a later instant becomes a `STALE` warning without triggering a mutation.
- `yandex-ai-hypothesis/v1` manages only explicit fenced JSON records with `HYPOTHESIS` / `DERIVED` provenance; unmanaged Markdown and prompt-like strings remain inert data.
- Restricted YAML parsing, atomic project writes, decision hash chaining, duplicate guards, future-timestamp guards, and a secret-like-key heuristic are implemented with no third-party Python packages.
- The repository validator and `CONTRACT_MATRIX.json` add four P1 infrastructure contracts; ARCHITECTURE, GETTING_STARTED, and SECURITY are synchronized in RU/EN.
- Project Memory is not write permission: a new consequential write still requires a new exact `preview_id`, later-turn human approval, and separate `--ack-bulk` for bulk/unknown scale. The secret heuristic is not represented as a complete DLP system.
- Release intent is repository-only: `.github/releases/release.json` contains `plugins: []`; plugin releases/tags are neither created nor retargeted.

...
Версия `2.2.0`. Service plugin для Яндекс Директа: кампании, Reports API, аудит, ключевые слова/минус-фразы, бюджеты, оптимизация и low-level API workflows.
...
Практический слой теперь начинается не с универсального KPI-порога, а с бизнес-цели, корректного объекта анализа и сопоставимой базы данных. Для оптимизации используйте последовательность `наблюдение → гипотеза → проверка → действие → критерий эффекта` и заранее задавайте success/stop/revert criteria.

`RARELY_SERVED`, низкий объём или единичный плохой KPI не являются самостоятельной командой удалить ключ. Сначала отделите реальный коммерческий интент от шума, проверьте совместимый период/атрибуцию и рассмотрите консолидацию совместимого редкого спроса. Изменение минус-фраз, ставок или бюджетов по-прежнему требует exact preview и отдельного later-turn approval.

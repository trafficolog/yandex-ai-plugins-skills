...
class Repository1010ReleaseSurfaceTests(unittest.TestCase):
    ...
    def test_bilingual_changelogs_preserve_repository_1_0_10(self):
        marker = "## [1.0.10] — 2026-09-05"
        for filename in ("CHANGELOG.md", "CHANGELOG.en.md"):
            with self.subTest(filename=filename):
                self.assertIn(marker, self.read(filename))

    def test_repository_1_0_10_release_notes_remain_historical(self):
        notes = self.read(".github/releases/1.0.10.md")
        self.assertIn("Repository 1.0.10", notes)

...
Текущая опубликованная версия репозитория — `1.5.0`. Версии плагинов развиваются независимо. Теги опубликованных релизов не переназначаются на новые коммиты. Для поколений, опубликованных после внедрения защиты неизменяемых релизов, GitHub-релиз сохраняет состояние `immutable`; более ранние релизы остаются в своём исходном историческом состоянии.
...
|---|---:|---|---|---|
| [`yandex-direct`](plugins/yandex-direct/) | 2.2.0 | сервисный | кампании, отчёты, ключевые слова, бюджеты, аудит | только после точного предварительного просмотра и отдельного подтверждения |
| [`yandex-metrika`](plugins/yandex-metrika/) | 2.2.0 | сервисный | аналитика, цели, атрибуция, Logs API, импорты | только после точного предварительного просмотра и отдельного подтверждения |
| [`yandex-webmaster`](plugins/yandex-webmaster/) | 2.2.0 | сервисный | индексация, запросы, переобход, карты сайта, фиды | только после точного предварительного просмотра и отдельного подтверждения |
| [`yandex-wordstat`](plugins/yandex-wordstat/) | 1.2.0 | сервисный | спрос, частотность, динамика, регионы, темы | нет опасных операций записи |
| [`yandex-search`](plugins/yandex-search/) | 1.1.0 | сервисный | поисковая выдача, позиции, конкуренты, кластеризация | нет |
...
### Практические рабочие контуры

Версия `1.5.0` усиливает пять сервисных плагинов практиками из регулярно используемых рабочих сценариев: бизнес-цель и правильный объект анализа фиксируются до метрик, сравнения строятся на совместимой базе, наблюдение отделяется от гипотезы, а действие получает явный критерий успеха или остановки.

- Direct не превращает редкий спрос и статус `RARELY_SERVED` в автоматическую команду удалить фразу;
- Metrika отделяет микрособытия от бизнес-конверсий и не теряет нулевые конверсии из знаменателя;
- Search использует smart snippets / info context только для ограниченного смыслового исследования, сохраняя обычный SERP-сбор основным режимом;
- Webmaster требует свежих evidence перед recrawl и не использует blanket recrawl как псевдорешение падения позиций;
- Wordstat сначала проверяет регион, коммерческий смысл и неоднозначный интент, а уже потом превращает спрос в рекомендации.

Источник практических паттернов закреплён точным commit SHA в release notes; внешняя практика дополняет, но не заменяет официальные API-контракты Яндекса и наши exact-preview safety gates.
...
В набор результата входят машиночитаемый `report.json`, автономный `report.html` и манифест с SHA-256. Уже созданный снимок не перезаписывается при конфликте, а возможные действия остаются только предложениями (`PREVIEW-ONLY`). Технические схемы артефактов — `seo-weekly-organic-report/v1` и `yandex-ai-artifact-manifest/v1`; HTML остаётся self-contained, то есть не зависит от внешних ресурсов для отображения сохранённого отчёта.

Подробнее: [`plugins/yandex-seo/README.md`](plugins/yandex-seo/README.md).

### Оценка качества моделей

Этап P3 добавил исполняемую инфраструктуру для проверки того, как разные AI-модели проходят одинаковые сценарии плагинов. Она нейтральна к поставщику модели (`provider-neutral`): конкретная модель подключается через адаптер, а правила оценки остаются общими.
...
Wordstat даёт данные о спросе, Search — проверку поисковой выдачи (SERP), Webmaster и Metrika — контекст существующего сайта. SEO объединяет их, не получая собственного транспорта к API Яндекса. В техническом контракте два последовательных уровня называются `Topical Architecture` и `Internal Linking`; в основном русском тексте используются их понятные эквиваленты — тематическая архитектура и внутренняя перелинковка.
...
- [`docs/reviews/2026-09-05-fable-round2-closure.md`](docs/reviews/2026-09-05-fable-round2-closure.md) — закрытие второго раунда FABLE;
- [`docs/reviews/2026-09-05-opus-codex-governance.md`](docs/reviews/2026-09-05-opus-codex-governance.md) — последнее датированное ревью по управлению и контрактам;
